...
import type {ExtendedMultifactorAuthenticationStatus, MultifactorAuthorizationFallbackProps} from './types';

function MultifactorAuthentication<T extends MultifactorAuthorizationFallbackScenario>({children, scenario, params}: MultifactorAuthorizationFallbackProps<T>) {
    ...
    const hasAccess = !!MultifactorAuthorizationFallback.wasRecentStepSuccessful && MultifactorAuthorizationFallback.isRequestFulfilled;
    ...
    const renderMultifactorAuthenticationInput = (factor: string, paramName: string) => (
        <InputModal
            onSubmit={(value) => {
                handleAuthorize({[paramName]: value});
            }}
    ...
    return (
        <>
            ...
            {MultifactorAuthorizationFallback.requiredFactorForNextStep === CONST.MULTI_FACTOR_AUTHENTICATION.FACTORS.VALIDATE_CODE &&
                renderMultifactorAuthenticationInput('ValidateCode', CONST.MULTI_FACTOR_AUTHENTICATION.FACTORS_REQUIREMENTS.VALIDATE_CODE.parameter)}
            {MultifactorAuthorizationFallback.requiredFactorForNextStep === CONST.MULTI_FACTOR_AUTHENTICATION.FACTORS.OTP &&
                renderMultifactorAuthenticationInput('OTPCode', CONST.MULTI_FACTOR_AUTHENTICATION.FACTORS_REQUIREMENTS.OTP.parameter)}
        </>
    );
}
