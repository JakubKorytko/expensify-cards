 ...
 */
function convertBiometricsParameterNameToFactor<T extends BiometricsScenario>(
  parameter: keyof BiometricsFactors<T>,
) {
  return Object.values(CONST.BIOMETRICS.FACTORS_REQUIREMENTS).find(
    (factor) => factor.parameter === parameter,
  )?.id as BiometricsFactor;
}
...
export {
  ...
  convertBiometricsParameterNameToFactor,
};
